...
### エンドポイント応答時間ベンチマークテスト

シードデータを投入した状態で、高負荷なエンドポイント（タイムライン、トレンド、検索、スレッドツリー等）の応答時間（Min, Avg, P50, P95, Max）を計測・レポートできます。

```bash
# デフォルト実行（SCALE=1, ITERATIONS=10）
make bench

# スケールや試行回数を指定して実行
make bench SCALE=2 ITERATIONS=20
```
